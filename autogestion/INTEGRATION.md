...
La lectura de laboratorio quedó aceptada en f47bb26. Se agregó creación y reconciliación SIRO, deshabilitada por defecto y validada con fixtures y una prueba manual real controlada. Ver [arquitectura SIRO](SIRO.md), [primera prueba](FIRST-SIRO-TEST.md) e [imputación Phantom](PHANTOM-PAYMENTS.md). SIRO intent created, SIRO payment confirmed y Phantom payment posted siguen siendo hitos distintos. El tercero está preparado detrás de una compuerta separada, con revalidación e idempotencia. La autenticación CRM real fue confirmada manualmente con `CRM_AUTH_OK`; la primera llamada real a `Imputar_Pago` sigue pendiente.
