...
La lectura de laboratorio quedó aceptada en f47bb26. Se agregó creación y reconciliación SIRO, deshabilitada por defecto y validada con fixtures y una prueba manual real controlada. Ver [arquitectura SIRO](SIRO.md), [primera prueba](FIRST-SIRO-TEST.md) e [imputación Phantom](PHANTOM-PAYMENTS.md). SIRO intent created, SIRO payment confirmed y Phantom payment posted siguen siendo hitos distintos. La autenticación CRM real fue confirmada con `CRM_AUTH_OK`; el preflight devolvió `READY_FOR_CONTROLLED_POST` y la primera llamada real se efectuó una sola vez. La relectura quedó `POST_UNCONFIRMED`, sin reintento automático: el saldo pasó de $121 a $0, pero REST/CRM todavía no dieron confirmación conjunta.

Revisión actual del 19/09/2026: 493 verificaciones con fixtures. Además de SIRO, cubren token CRM independiente del REST, `Consultar_Impagos` exacto, preflight sin escritura, contradicciones REST/CRM/SIRO, acknowledgements CRM, timeout posterior al posible envío, ausencia de reintento ciego y estado `ALREADY_SETTLED`. QA desktop y mobile con checkout interceptado, más prueba manual real de cancelación y pago confirmado. La UI separa Facturas y Movimientos y no ofrece reconciliación manual para estados terminales. Sin errores JavaScript ni desborde horizontal. Sintaxis PHP/JS y chequeo local de secretos correctos.

El token de API Rest y el token CRM son contratos separados, con cachés privadas distintas. `inspect-phantom-crm.php` y la escritura usan `PhantomCrmHttp`; no existen dos autenticaciones CRM divergentes. El preflight `inspect-payment-posting-preflight.php` funciona con las compuertas deshabilitadas, localiza un único intento confirmado en el runtime y consulta SIRO, API Rest y CRM sin invocar `Imputar_Pago`.
...
Ajuste de presentación solicitado: se oculta el Detalle técnico sin modificar los datos recibidos. En pantalla se quitan del plan la fecha técnica inicial y los segmentos internos de Phantom con formato `CODIGO ($) -`, incluidos `RES`, `EMP`, `COM` y `MUNI`; del domicilio se ocultan lote, manzana, referencia y barrio agregados por CRM. El selector muestra solo domicilio y plan, sin número de contrato. Gestionar mi servicio queda visible y sus modificaciones continúan deshabilitadas en modo Phantom. La velocidad separada se omite cuando Phantom no entrega un valor confirmado. La prueba de velocidad se integra dentro del portal con LibreSpeed local y un servidor de medición HTTPS configurado por el operador. Detalles, actualización de conexión, adicionales y límites de Wi-Fi en [MI-SERVICIO.md](MI-SERVICIO.md).
...
## Ingreso global del portal

Los usernames numéricos positivos son candidatos por defecto; `allowed_idas` y
`service_login_idas` no restringen el login. Phantom confirma identidad y
credenciales exactas antes de crear sesión. Las consultas posteriores quedan
limitadas a `authorized_services` y `selected_ida`, construidos por el servidor.
Aliases no numéricos confirmados pueden declararse en `login_users`. SIRO,
imputación, tickets y SOAP conservan sus compuertas independientes.
