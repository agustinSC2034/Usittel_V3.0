...
function render() {
  ...
  if (runtime.mode === 'phantom') {
    ...
    hint.textContent = route === 'login' ? 'La recuperación de contraseña todavía no está habilitada.' : 'Modo lectura. Las acciones de pago y modificación todavía no están habilitadas.';
...
document.addEventListener('click', async event => {
  ...
  if (action === 'pay') {
    ...
    return openDialog('Pagar en SIRO', `<p>El pago se realiza en el portal de SIRO. No ingresás datos de tu tarjeta en Mi USITTEL.</p><div class="document-summary"><h3>${item.period}</h3><p class="amount">${money(item.amount)}</p><p class="muted">Vencimiento ${item.due}</p></div><p class="demo-notice">Esta es una vista de prueba: no se abrirá SIRO ni se realizará ningún cobro.</p><div class="dialog-actions">${button('Continuar a SIRO', '', { iconName: 'external-link', attrs: 'disabled aria-describedby="payment-note"' })}${button('Cerrar', 'close', { secondary: true })}</div><p id="payment-note" class="field-hint">El enlace de pago todavía no está habilitado.</p>`);
  }
...
async function boot() {
  ...
  try {
    ...
    await initialize(session.mode); authenticated = session.authenticated;
    document.querySelector('.demo-strip').textContent = session.mode === 'demo' ? 'Vista de prueba · Datos de ejemplo' : 'Desarrollo local · Cuenta de laboratorio · Solo lectura';
