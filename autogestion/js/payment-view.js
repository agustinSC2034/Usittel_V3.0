...
export function paymentPanel() {
  ...
  const registered = runtime.paymentHistoryItems.map(item => `<div class="commercial-option"><h3>Pago registrado</h3><p>${date(e(item.date))} · ${money(item.amount)}</p><p class="field-hint">${e(item.method)} · Período ${e(item.period)}</p>${item.downloadAvailable ? button('Descargar comprobante', 'download-payment-receipt', { secondary: true, iconName: 'download', attrs: `data-payment-id="${e(item.id)}"` }) : ''}</div>`).join('');
