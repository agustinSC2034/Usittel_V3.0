...
(async()=>{
  ...
  check('cURL distingue verificación, handshake y CA inválida sin red',()=>{assert.equal(command.status,0,command.stderr);const diagnostic=JSON.parse(command.stdout);assert.equal(diagnostic.handshake,'PHANTOM_TLS_HANDSHAKE');assert.equal(diagnostic.verify,'PHANTOM_TLS_VERIFY');assert.equal(diagnostic.ca_file,'PHANTOM_CA_FILE');assert.equal(diagnostic.missing_ca,'PHANTOM_CA_FILE');});
