...
(async()=>{
  ...
  clearRate();fs.writeFileSync(config,settings('demo'));const demo=jar();r=await login(demo,'agustin.demo','usittel-demo');check('demo explícito en servidor',()=>assert.equal(r.status,200));
