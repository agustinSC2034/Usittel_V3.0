...
$nilEnvelope=soapEnvelopeShape('<?xml version="1.0"?><soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"><soap:Body><consulta_perfilesResponse><return xsi:nil="true"/></consulta_perfilesResponse></soap:Body></soap:Envelope>');
checkOp($nilEnvelope['format']==='soap_xml' && $nilEnvelope['operation']==='consulta_perfilesResponse');
checkOp($nilEnvelope['result']['nil'] && $nilEnvelope['result']['text_state']==='empty' && $nilEnvelope['result']['element_children']===0);
$valueEnvelope=soapEnvelopeShape('<?xml version="1.0"?><Envelope><Body><consulta_perfilesResponse><return><item>fixture-private</item></return></consulta_perfilesResponse></Body></Envelope>');
checkOp(!$valueEnvelope['result']['nil'] && $valueEnvelope['result']['text_state']==='nonempty' && $valueEnvelope['result']['element_children']===1);
checkOp(!str_contains(json_encode($valueEnvelope),'fixture-private'));
checkOp(soapEnvelopeShape('<!DOCTYPE fixture [<!ENTITY x "private">]><Envelope/>')['format']==='invalid_xml');
