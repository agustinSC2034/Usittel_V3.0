...
Default provider: https://nominatim.openstreetmap.org/search . Policy: https://operations.osmfoundation.org/policies/nominatim/ . No autocomplete or background enumeration. Each submitted recognized address starts with its original number. If no numbered match is returned, the client tries offsets +1, -1, +2, -2, +3, -3, +5, -5, +10, -10 sequentially, waiting 1.2 seconds between completed requests. Errors stop the sequence; editing cancels it. The gateway serializes uncached traffic across visitors, enforces at least 1.1 seconds between upstream starts and applies a 30-second backoff on provider failure. It returns 429 when busy instead of queuing requests. Cache hits remain available during backoff. Cache has at most 256 slots, 24-hour positive and 10-minute empty-result TTLs. Cache stores a hashed query key and minimal map result, not raw submitted query or user IP.
...
Client timeout is nine seconds per attempt, upstream timeout six seconds. Input changes cancel requests and invalidate old results. Exact markers require matching city, street and house number. If the same response contains a numbered result on the same street within ten address numbers, the closest number can provide a map marker. Successful locations display only the marker, without a popup or a status caption. Number proximity does not guarantee geographic distance. Unnumbered street centroids are not used as household references. The bounded fallback stops at the first matching numbered reference; it never changes commercial coverage. A different house is never relabeled with the requested number.
