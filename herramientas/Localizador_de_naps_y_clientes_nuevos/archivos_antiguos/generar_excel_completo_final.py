#!/usr/bin/env python3
"""
Script FINAL para generar Excel completo con TODOS los ~3300 clientes
Incluye geolocalización y asignación de NAPs cercanas
"""
...
class ProcesadorClientesFinal:
    def __init__(self):
        self.geolocator = Nominatim(user_agent="usittel_final_v1", timeout=10)
    ...
    def cargar_clientes_completo(self):
        """Carga TODOS los clientes de ambas zonas"""
        archivo = "base de datos copia.xlsx"
        
        print(f"📂 Cargando clientes desde: {archivo}")
        
        # Leer ZONA 1
        zona1 = pd.read_excel(archivo, sheet_name='ZONA 1')
        print(f"📊 ZONA 1 leída: {len(zona1)} filas")
        
        # Limpiar ZONA 1 - usar 'Unnamed: 1' como nombre
        zona1_clean = zona1[
        ...
        print(f"📊 ZONA 1 limpia: {len(zona1_clean)} clientes")
        print(f"📊 ZONA 2 limpia: {len(zona2_clean)} clientes") 
        print(f"📊 Total clientes unidos: {len(clientes_todos)}")
        print(f"📊 Clientes que NO contrataron: {len(clientes_filtrados)}")
        print(f"📊 Distribución filtrados por zona:")
    ...
    def geocodificar_todos_los_clientes(self, clientes):
        """Geocodifica TODOS los clientes"""
        print(f"\n🌍 Iniciando geocodificación de {len(clientes)} clientes...")
    ...
    def encontrar_naps_cercanas(self, clientes_coords, naps):
        ...
        print(f"\n📏 Buscando NAPs cercanas para {len(clientes_coords)} clientes...")
        ...
        print(f"📊 Clientes con NAPs cercanas (≤{RADIO_BUSQUEDA}m): {len(resultados_con_nap)}")
        
        return pd.DataFrame(resultados_con_nap)
    
    def generar_excel_final_completo(self, clientes_originales, clientes_coords, clientes_con_nap):
        """Genera Excel final con TODOS los clientes"""
        print(f"\n📊 Generando Excel final con {len(clientes_originales)} clientes...")
        ...
        nombre_archivo = f"clientes_TODOS_final_{timestamp}.xlsx"
...
def main():
    """Función principal - Procesa TODOS los clientes"""
    print("🚀 SCRIPT FINAL - Procesando TODOS los ~3300 clientes")
    print("="*60)
    
    procesador = ProcesadorClientesFinal()
    
    # 1. Cargar todos los clientes
    print("\n📂 PASO 1: Cargando clientes...")
    clientes = procesador.cargar_clientes_completo()
    ...
    # 3. Geocodificar TODOS los clientes
    print("\n🌍 PASO 3: Geocodificando clientes...")
    respuesta = input(f"¿Procesar TODOS los {len(clientes)} clientes? (s/n): ").lower()
    
    if respuesta != 's':
        print("⏸️ Proceso cancelado por el usuario")
        return
    
    clientes_coords = procesador.geocodificar_todos_los_clientes(clientes)
    ...
    archivo_excel, df_final = procesador.generar_excel_final_completo(
