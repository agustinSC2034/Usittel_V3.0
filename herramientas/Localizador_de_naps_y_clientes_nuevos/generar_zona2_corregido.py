#!/usr/bin/env python3
"""
Script CORREGIDO para procesar ZONA 2 con cálculos de distancia correctos
"""
...
CACHE_GEOCODING = "cache_geocoding_zona2_corregido.json"

class ProcesadorZona2Corregido:
    def __init__(self):
        self.geolocator = Nominatim(user_agent="usittel_zona2_corregido_v1", timeout=10)
    ...
    def encontrar_naps_cercanas_corregido(self, clientes_coords, naps):
        """VERSIÓN CORREGIDA: Encuentra NAP más cercana para cada cliente geocodificado"""
        print(f"\n📏 Buscando NAPs cercanas para ZONA 2 (VERSIÓN CORREGIDA)...")
        
        clientes_geocodificados = clientes_coords[clientes_coords['geocodificado']].copy()
        
        resultados_con_nap = []
        
        print(f"🔍 Procesando {len(clientes_geocodificados)} clientes geocodificados...")
        ...
        for idx, cliente in clientes_geocodificados.iterrows():
            ...
            # PASO 1: Encontrar la NAP más cercana SIN filtro de distancia
            for _, nap in naps.iterrows():
                ...
                # Encontrar la más cercana independientemente de la distancia
                if distancia < distancia_minima:
                    distancia_minima = distancia
                    nap_mas_cercana = nap
            
            # PASO 2: Solo incluir si la NAP más cercana está dentro del radio permitido
            if nap_mas_cercana is not None and distancia_minima <= RADIO_BUSQUEDA:
                ...
                # DEBUG: Mostrar casos específicos
                if cliente['nombre'] and 'MILANESI' in cliente['nombre'].upper():
                    print(f"🔍 DEBUG - Cliente: {cliente['nombre']}")
                    print(f"    Dirección: {cliente['direccion_original']}")
                    print(f"    Coordenadas cliente: {cliente['lat']}, {cliente['lon']}")
                    print(f"    NAP encontrada: {nap_mas_cercana['nombre_nap']}")
                    print(f"    Coordenadas NAP: {nap_mas_cercana['Latitud']}, {nap_mas_cercana['Longitud']}")
                    print(f"    Distancia calculada: {distancia_minima:.1f}m")
                    print(f"    ¿Dentro del radio? {distancia_minima <= RADIO_BUSQUEDA}")
        
        print(f"📊 Clientes ZONA 2 con NAPs cercanas (≤{RADIO_BUSQUEDA}m): {len(resultados_con_nap)}")
        
        return pd.DataFrame(resultados_con_nap)
    
    def generar_excel_zona2_corregido(self, clientes_originales, clientes_coords, clientes_con_nap):
        """Genera Excel final con TODOS los clientes de ZONA 2 - VERSIÓN CORREGIDA"""
        print(f"\n📊 Generando Excel ZONA 2 CORREGIDO con {len(clientes_originales)} clientes...")
        ...
        nombre_archivo = f"clientes_ZONA2_CORREGIDO_{timestamp}.xlsx"
        ...
        print(f"\n✅ Excel ZONA 2 CORREGIDO generado: {nombre_archivo}")
...
def main():
    """Función principal - Procesa ZONA 2 con algoritmo CORREGIDO"""
    print("🚀 PROCESANDO ZONA 2 - VERSIÓN CORREGIDA")
    print("="*55)
    print("🔧 Correcciones aplicadas:")
    print("   ✅ Algoritmo de búsqueda de NAPs corregido")
    print("   ✅ Cálculo de distancias verificado")
    ...
    # 3. Geocodificar ZONA 2 (reutilizar cache si existe)
    ...
    # 4. Encontrar NAPs cercanas CON ALGORITMO CORREGIDO
    print("\n📏 PASO 4: Buscando NAPs cercanas (ALGORITMO CORREGIDO)...")
    clientes_con_nap = procesador.encontrar_naps_cercanas_corregido(clientes_coords, naps)
    
    # 5. Generar Excel final corregido
    print("\n📊 PASO 5: Generando Excel ZONA 2 CORREGIDO...")
    archivo_excel, df_final = procesador.generar_excel_zona2_corregido(
        clientes, clientes_coords, clientes_con_nap
    )
    
    print(f"\n🎉 ZONA 2 CORREGIDA COMPLETADA!")
    ...
    print(f"\n🔍 Verificar especialmente el caso de MILANESI en el Excel")
    print(f"✅ Si está correcto, aplicaremos la misma lógica a ZONA 1")
