#!/usr/bin/env python3
"""
Script NUEVO para procesar ZONA 2 con algoritmo limpio desde cero
"""
...
class ProcesadorZona2:
    def __init__(self):
        self.geolocator = Nominatim(user_agent="usittel_zona2_nuevo", timeout=15)
        self.cache_geocoding = {}
        self.cache_file = "cache_zona2_nuevo.json"
        self.cargar_cache()
    ...
    def limpiar_direccion(self, direccion):
        """Limpiar y normalizar dirección - SOLO calle y número"""
        if pd.isna(direccion):
            return None
        
        direccion = str(direccion).strip()
        
        # Limpiar referencias a Tandil explícitas
        direccion = direccion.replace('TANDIL', '').strip()
        direccion = direccion.replace('Tandil', '').strip()
        
        # CORRECCIÓN CRÍTICA: Remover departamentos, casas, locales, etc.
        # Esto corrige el problema de "ALSINA 956 - DTO. 4" → "ALSINA 956"
        import re
        
        # Lista de sufijos problemáticos para geocodificación
        sufijos_remover = [
            # Departamentos y similares
            r'\s*[-–]\s*(DPTO\.?|DTO\.?|DEPTO\.?|DEPARTAMENTO).*$',
            r'\s+(DPTO\.?|DTO\.?|DEPTO\.?|DEPARTAMENTO).*$',
            
            # Casas, locales, oficinas
            r'\s*[-–]\s*(CASA|LOCAL|OFICINA|TALLER).*$',
            r'\s+(CASA|LOCAL|OFICINA|TALLER).*$',
            ...
            # Planta baja/alta
            r'\s*[-–]\s*(PB|PLANTA\s+BAJA|PLANTA\s+ALTA).*$',
            r'\s+(PB|PLANTA\s+BAJA|PLANTA\s+ALTA).*$',
            
            # Frente/fondo
            r'\s*[-–]\s*(FRENTE|FONDO|CONTRAFRENTE).*$',
            r'\s+(FRENTE|FONDO|CONTRAFRENTE).*$'
        ]
        
        # Aplicar todas las limpiezas
        for patron in sufijos_remover:
            direccion = re.sub(patron, '', direccion, flags=re.IGNORECASE)
        
        # Normalizar espacios múltiples
        direccion = ' '.join(direccion.split())
        
        # Verificar que tengamos algo útil
    ...
    def cargar_datos(self):
        ...
        # Cargar NAPs finales corregidas manualmente
        naps_file = "excels_con_los_datos_de_partida/archivo_final_naps.xlsx"
        try:
            self.df_naps = pd.read_excel(naps_file)
            print(f"✅ NAPs finales corregidas: {len(self.df_naps)} registros")
            print(f"   Columnas: {list(self.df_naps.columns)}")
            
            # Verificar que tengamos coordenadas
            if 'Latitud' in self.df_naps.columns and 'Longitud' in self.df_naps.columns:
                coords_validas = (self.df_naps['Latitud'].notna() & self.df_naps['Longitud'].notna()).sum()
                print(f"   📍 NAPs con coordenadas válidas: {coords_validas}/{len(self.df_naps)} ({coords_validas/len(self.df_naps)*100:.1f}%)")
            else:
    ...
    def buscar_nap_cercana(self, lat_cliente, lon_cliente, direccion_cliente):
        ...
        for i, nap in self.df_naps.iterrows():
            ...
            # Usar coordenadas corregidas del archivo final
            lat_nap = nap['Latitud'] if 'Latitud' in nap else None
            lon_nap = nap['Longitud'] if 'Longitud' in nap else None
            
            if lat_nap is None or lon_nap is None or pd.isna(lat_nap) or pd.isna(lon_nap):
                continue
            
            # Verificar que la NAP tenga capacidad disponible (≤ 30% ocupación)
            if isinstance(ocupacion_porcentaje, (int, float)) and ocupacion_porcentaje > 0.3:
                continue
    ...
    def guardar_excel(self, resultados):
        ...
        filename = f"clientes_ZONA2_NUEVO_{timestamp}.xlsx"
    ...
    def ejecutar(self):
        ...
        print("🚀 INICIANDO PROCESAMIENTO ZONA 2 - ALGORITMO NUEVO")
        print("="*60)
